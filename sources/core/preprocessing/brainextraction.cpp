 ...
 *
 */
BrainExtractor::BrainExtractor(AnimaWrapper *wrapper, const QString &atlasImage, QObject *parent)
...
void BrainExtractor::requestCancel() 
{
 ...
 */
void BrainExtractor::runCommand(const QStringList &command) 
{
 ...
 */
QString BrainExtractor::run(const QString &imgPath, const QString &prefix) 
{
    try 
    {
        QElapsedTimer timer;
...
/**
 * @brief Optimized brain extraction for a modality that shares the same reference
 * family (and therefore the same target atlas space) as an already-processed
 * "anchor" modality. Instead of recomputing the costly dense non-linear
 * registration (animaDenseSVFBMRegistration), this performs a fast rigid
 * registration onto the anchor's own patient-space image, then reuses the
 * anchor's affine + dense transform chain to reach atlas space.
 * @param imgPath Path to the modality to process.
 * @param prefix Output file prefix (temp/working directory + base name).
 * @param anchorImgPath Path to the anchor modality's own (patient-space) image.
 * @param anchorAffTrsfPath Path to the anchor's affine transform (*_aff_tr.txt).
 * @param anchorNlTrsfPath Path to the anchor's dense transform (*_nl_tr.nrrd).
 * @return Path to the brain-extracted image for this modality.
 */
QString BrainExtractor::runWithExistingTransform(const QString &imgPath, const QString &prefix, const QString &anchorImgPath, const QString &anchorAffTrsfPath, const QString &anchorNlTrsfPath) {
    try
    {
        QElapsedTimer timer;
        timer.start();
        emit progress(0.0f, "Starting optimized brain extraction (reusing anchor transform)");

        QString maskedBrain = prefix + "_BET.nii.gz";
        QStringList command;

        // --- Fast rigid registration onto the anchor's own patient-space image ---
        emit progress(0.2f, "Rigid registration onto anchor modality");
        command << "animaPyramidalBMRegistration"
                << "-m" << anchorImgPath << "-r" << imgPath << "-o" << (prefix + "_rig_to_anchor.nrrd") << "-O" << (prefix + "_rig_to_anchor_tr.txt") << "--sp" << "3";
        command += m_pyramidOption;
        runCommand(command);

        qDebug() << "Rigid-to-anchor registration completed in" << timer.elapsed() / 1000.0 << "seconds";
        timer.restart();

        // --- Combine [rigid-to-anchor] + [anchor's affine] + [anchor's dense] into a single transform series ---
        emit progress(0.5f, "Composing transform series with anchor");
        command.clear();
        command << "animaTransformSerieXmlGenerator"
                << "-i" << (prefix + "_rig_to_anchor_tr.txt") << "-i" << anchorAffTrsfPath << "-i" << anchorNlTrsfPath << "-o" << (prefix + "_nl_tr.xml");
        runCommand(command);

        qDebug() << "Combined transform series generated in" << timer.elapsed() / 1000.0 << "seconds";
        timer.restart();

        // --- Apply ICC mask using the combined transform ---
        emit progress(0.8f, "Applying ICC mask");
        command.clear();
        command << "animaApplyTransformSerie"
                << "-i" << m_iccImage << "-t" << (prefix + "_nl_tr.xml") << "-g" << imgPath << "-o" << (prefix + "_rough_brainMask.nrrd") << "-n" << "nearest";
        runCommand(command);
        ...
        command.clear();
        command << "animaMaskImage"
                << "-i" << imgPath << "-m" << (prefix + "_rough_brainMask.nrrd") << "-o" << (prefix + "_rough_masked.nrrd");
        runCommand(command);
        ...
        emit progress(1.0f, "Optimized brain extraction finished");
        emit finished(maskedBrain);
    ...
    {
        qDebug() << "Optimized brain extraction failed:" << e.what();
        throw std::runtime_error(std::string("Optimized brain extraction failed: ") + e.what());
    }
}
